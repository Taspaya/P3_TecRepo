...
Renderer::Renderer()
{
	...
	if (Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 1024) == -1) {
		throw "Uneable to inicialize SDL_mixer audio systems";
	}
...
void Renderer::LoadSong(const std::string &id, const std::string &path) {
	/*Mix_Music *cancion{ Mix_LoadMUS(path.c_str()) };
	if (cancion == nullptr) throw "No se ha podido cargar la cancion";
	m_songData[id] = cancion;*/
}
